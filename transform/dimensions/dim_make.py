...
def transform_dim_make(stg_make_df: DataFrame) -> DataFrame:
    clean_df = (
        stg_make_df
        .filter(col("make").isNotNull() & (trim(col("make")) != ""))
        .withColumn("_clean_id", col("make_id").cast("integer"))
        .withColumn("_clean_name", initcap(trim(col("make"))))
    )
    ...
    window_key = Window.orderBy("Make_Id_V2")
    res_df = (
        step5_df
        .withColumn("Make_Key", row_number().over(window_key))
        .select(
            col("Make_Key"),
            col("_clean_id").alias("Make_Id"),
            col("_clean_name").alias("Make_Name"),
            col("Make_Id_V2").cast("integer").alias("Make_Id_V2")
        )
    )
